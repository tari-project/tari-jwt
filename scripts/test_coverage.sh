...
LLVM_PROFILE_FILE="./cov_raw/tari_jwt-%m.profraw"
...
cargo profdata -- \
  merge -sparse ./cov_raw/tari_jwt-*.profraw -o ./cov_raw/tari_jwt.profdata
...
cargo cov -- \
  export \
    ...
    --instr-profile=cov_raw/tari_jwt.profdata \
    $files \
    > cov_raw/tari_jwt.lcov
...
cargo cov -- \
  show \
    ...
    --instr-profile=cov_raw/tari_jwt.profdata \
    $files \
    > cov_raw/tari_jwt.txt

if [ -z ${SKIP_HTML+x} ]; then
  genhtml -o coverage_report cov_raw/tari_jwt.lcov
else
